...
* **[REF: MED-11] Strict Line-Break Delimiters:** To preserve valid internal punctuation within URIs (such as commas in file names) and metadata grammar, the parser utilizes a strict, simplified line-break regex (`/\r\n|\n|\r/`). This comprehensively intercepts all cross-platform newline injections (Windows, Unix/Sheets `CHAR(10)`, legacy Mac) while abandoning greedy legacy delimiter fallbacks (`,`, `|`), guaranteeing zero destructive splits on valid string content.
* **[REF: MED-14] Absolute Structural Validation:** Component prop inputs rigorously track the upstream parent injection state (`{ activeSlide }` vs `{ data }`) to prevent lethal undefined-reference voids. Data parsing strictly isolates cross-platform line-breaks (`/\r?\n/`) for array segmentation, fully shielding file path commas and natural string grammars from catastrophic URL fragmentation.
* **[REF: MED-18] The Omni-Splitter Matrix:** To combat unpredictable cross-platform CSV string encoding (where upstream exports may inject lone carriage returns `\r`, standard line-feeds `\n`, or literal escaped text artifacts `\\n`), the flattening matrix employs a universal regex interceptor (`/[\r\n]+|\\n/`). This guarantees catastrophic string concatenation is prevented across all OS environments, mathematically ensuring 1:1 array synchronization.
...
* **[REF: TL-UI-05] Hexagon Whitespace Calibration:** The `.event-block` geometry retains its absolute left-anchor vertical line spanning the swimlane height for structural clarity. Internal vertical padding (`padding: 2px 12px`) is minimized, unlocking exactly 8 additional pixels of rendering height to flawlessly accommodate 3-line title string wraps within native lane bounds.
* **[REF: TL-ANIM-01] 3-Step Navigation Choreography:** Timeline slide transitions utilize a delayed React state machine to eliminate render whiplash. 1) Native `.active` state swap (instant). 2) Smooth pan to active coordinate at current zoom level. 3) 400ms delayed cluster zoom recalculation paired with a synchronous `behavior: 'auto'` scroll snap to preserve visual centering as the timeline width mutates.
* **[REF: TL-UI-06] Gregorian Semantic Axis Engine:** Replaced static millisecond-addition math with native `Date.setMonth() / Date.setDate()` loops. This guarantees chronological absolute accuracy (compensating for 28/31-day months and leap years). Contextual minor ticks seamlessly cascade based on available pixels (`pxPerMinor > 45`), generating localized typography (Months, Days, or HH:MM) tied natively to the generated Gregorian scale.
* **[REF: UI-NUM-02] Integer HUD Scalars:** To eliminate visual clutter, all spatial and temporal zoom HUD indicators apply an absolute `Math.round()` function, displaying clean whole-number integers (e.g., `2x` instead of `2.3x`) while the underlying engine retains high-precision floating-point math.
* **[REF: TL-UI-09] Absolute Context Anchors:** The timeline's dynamic Gregorian axis generator enforces a spatial rule: the very first rendered tick on the far-left edge of the viewport bypasses scale constraints to output a full absolute date string (`DD/MM/YYYY HH:MM`). This guarantees immediate chronological context regardless of how far the user has zoomed into the micro-scale (e.g., `HH:MM`).
* **[REF: TL-ANIM-05] Synchronous Transition Engine:** The timeline utilizes Preact's `useLayoutEffect` to recalculate track width and mutate the `scrollLeft` position synchronously before the browser paints the next frame. This mathematical alignment permanently eliminates the DOM "ghost-pan" flicker previously caused by asynchronous `useEffect` rendering loops.
* **[REF: TL-UI-12] Native DOM Debouncer Awakening:** To bridge the disconnect between React state and manual DOM scrolling, the `.timeline-scroll-wrapper` employs a 100ms debounced `onScroll` event listener. This guarantees the viewport prediction engine continuously tracks the user's manual X-axis navigation, accurately repositioning absolute semantic dates regardless of cursor isolation.
* **[REF: TL-UI-13] The Typographic Safe-Zone:** To prevent rendering collisions, the `firstVisibleMajorTick` triggers a strict CSS transform override (`translateX(-1px)`). This locks the visual hash mark to the absolute pixel coordinate while cascading the absolute date string (`DD/MM/YYYY`) 100% to the right. The engine defines a `120px` exclusion boundary proceeding this coordinate; any subsequent minor ticks calculated within this spatial zone are structurally aborted, guaranteeing zero typographic overlap.
* **[REF: TL-UI-14] Absolute Y-Axis Targeting:** Removed the arbitrary 33px baseline calculation for vertical anchor structs. `distanceToAxis` strictly calculates `(containerHeight - 28) - topPos`, ensuring the active line drops precisely to the 0px floor of the timeline axis. Time-span trapezoids drop 5px accordingly, resting their base flush on the axis floor, and their extraneous right-side vertical boundaries have been deleted to favor an unbroken `drop-shadow` translucency matrix.
...
* **[REF: MED-15] DOM Event Propagation Routing:** Re-established strict separation of visual state and interaction states. `<img>` nodes reside in pure Flexbox `div` wrappers, eliminating anchor-tag click-hijacking. Outbound URI navigation is explicitly isolated to a dedicated `[↗]` node dynamically aligned to the flex-end boundary of the 1:1 synchronized metadata terminal.
* **[REF: MED-19] Hybrid Telemetry & Typographic Architecture:** The engine enforces a strict bifurcation of interaction states. Visual nodes (`<img>`) are strictly stripped of `<a>` wrappers to permanently preserve overlay click-zone event propagation (swiping). Conversely, metadata nodes (`mediaCaption` and `mediaCredit`) completely restore their native absolute `<a>` wrappers, allowing users to execute direct typographic hyperlinks to the source URIs without disrupting the carousel's geometric bounds.
