...
[v8.10.8] - TimelineScrubber v26.10.0
    ...
    ✓ [Browser Compatibility & "Black Line" Fix] Resolved a critical rendering bug where certain browsers interpreted nested CSS color variables as syntax errors, resulting in solid black timeline drop-lines. The engine now uses bulletproof, standard hex fallbacks.
    ✓ [Magnetic HUD & Anti-Clipping] Fixed a geometric flaw where tooltips for long-duration events would attempt to render entirely off-screen if the event's start date was out of view. The tooltip now acts as a magnetic HUD, anchoring strictly to your cursor and intelligently dodging the screen edges so it is always 100% visible.
    ✓ [Luminous Heat-Glow] Enhanced the visual density markers for overlapping events. Instead of applying a dark shadow that created visual "mud," the timeline now emits a luminous, high-contrast sub-pixel glow (e.g., Soft Cyan for Blue pins) to accurately simulate light radiation.
...
[v8.10.6] - Unsanctioned architecture. Retracted prior to deployment. Valid spatial and chronological physics rolled into v8.10.7.
*** This is not always noted in similar cases ***
...
* **Network Fetch Architecture Pivot:** Injected `updateWhenIdle: true` directly into the `L.tileLayer` and `L.tileLayer.wms` WebGL constructors for all historical overlay maps. This absolutely forbids Leaflet from firing any `allmaps.xyz` IIIF proxy requests while the camera is executing its 2.5-second `flyToBounds` flight path. By restricting network HTTP requests strictly to the exact millisecond the map comes to a dead stop, the browser's internal engine is prevented from aggressively aborting pending tile fetches (`NS_BINDING_ABORTED`), permanently stabilizing the connection pipeline for slow, live-warping proxy servers like Princeton's IIIF cloud.
* **UI Geometry Lock:** Audited the `<polygon>` node array on the Map Layers button SVG to guarantee it mathematically matches the exact geometric stack parameters found in the original `v6` master blueprint. 
...
* **MarkerCluster Null Syntax Fixed:** Resolved a fatal `TypeError: children.getLatLng is not a function` boundary exception. Leaflet's `getAllChildMarkers()` naturally returns an array object which lacks a top-level coordinate hook. The `iconCreateFunction` interception logic now explicitly accesses index `0` of the children array (`children.getLatLng()`) before executing mathematical identity comparisons.
* **Pin Coordinate Alignment Restored:** Re-injected `iconSize: [1, 2]` and `iconAnchor: [2, 3]` across all marker constructors to permanently fix the pin head offset geometry bug. Tooltips offsets were simultaneously restored to `offset: [0, -34]` to keep labels hovering flawlessly above the anchored pins.
...
**Global Styles (CSS):**
*   **[UI-51] 50/50 Split Reset:** Modified the root `--primary-split` variable from `55%` down to `50%`, enforcing a perfectly symmetrical initial geometry between the Narrative and Visual columns on wide viewports.
*   **[UI-179] Inner Boundary Cages:** Injected strict `max-width: 100%; box-sizing: border-box; overflow: hidden;` rules directly into the `#app-layout` and `.timeline-pane` nodes. This stops the internal flex grids from physically pushing the absolute root container outward, fully sealing the phantom grey/black browser margins.

**ContentSlider (v4.4.1):**
*   **[CRASH-04] Transpiler Cleansing:** Eradicated invalid `{/* */}` JSX comments from the `htm` template literal, terminating the bug that caused literal engine instructions to render as physical text blocks in the UI.
*   **[UI-49] Typographical Alignment Matrix:** Remapped the padding architecture. The 55px Maximize button "keep-out zone" is now applied to the top-level sticky header wrapper, allowing the internal `85ch` envelope to securely center itself. This mathematically guarantees the Ribbon and the Title lock flush to the exact same left-axis, while symmetrically overhanging the `65ch` text block.
